...
#table data_provider
CREATE TABLE `content_bowl`.`DATA_PROVIDER` (
  `ID_DATA_PROVIDER` INT NOT NULL,
  `DS_MNEMONIC` VARCHAR(45) NOT NULL,
  `DS_DESCRIPTION` VARCHAR(256) NOT NULL,
  `DS_PARSER` VARCHAR(256) NOT NULL,
  `IS_PUBLIC` CHAR(1) NOT NULL DEFAULT 'Y',
  PRIMARY KEY (`ID_DATA_PROVIDER`),
  UNIQUE INDEX `DS_MNEMONIC_UNIQUE` (`DS_MNEMONIC` ASC));

#table stream
CREATE TABLE `content_bowl`.`STREAM` (
  `ID_STREAM` INT NOT NULL AUTO_INCREMENT,
  `ID_TENANT` INT NOT NULL,
  `ID_DATA_PROVIDER` INT NOT NULL,
  `DS_ACCESS_KEY` VARCHAR(256) NULL,
  PRIMARY KEY (`ID_STREAM`),
  INDEX `ID_TENANT_idx` (`ID_TENANT` ASC),
  INDEX `ID_DATA_PROVIDER_idx` (`ID_DATA_PROVIDER` ASC),
  CONSTRAINT `ID_TENANT`
    FOREIGN KEY (`ID_TENANT`)
    REFERENCES `content_bowl`.`TENANT` (`ID_TENANT`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `ID_DATA_PROVIDER`
    FOREIGN KEY (`ID_DATA_PROVIDER`)
    REFERENCES `content_bowl`.`DATA_PROVIDER` (`ID_DATA_PROVIDER`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);

#table author
CREATE TABLE `content_bowl`.`AUTHOR` (
